...
class PrePage(ctk.CTk):
    ...
    def apply_step(self, step_name):
        ...
        if step_name == "Simple Imputer":
            ...
            cat_cols = self.processed_df.select_dtypes(include=['object', 'category']).columns.tolist()

            if not cat_cols:
                print("No categorical columns to encode.")
            else:
                encoder = OneHotEncoder(sparse_output=False)
                encoded_array = encoder.fit_transform(self.processed_df[cat_cols].astype(str))
                encoded_df = pd.DataFrame(encoded_array, columns=encoder.get_feature_names_out(cat_cols), index=self.processed_df.index)

                # Drop original categorical columns and add encoded columns
                self.processed_df.drop(cat_cols, axis=1, inplace=True)
                self.processed_df = pd.concat([self.processed_df, encoded_df], axis=1)
